...
class LogParser():
    ...
    reParsing = re.compile(r'python|bash')
    reStatus = ''

    logroot = "http://tinderbox.mozilla.org/showlog.cgi?log=Firefox-Unittest/"
    tests = dict()
    ...
    def getTestType(self, text):
        ...
        reReftest = re.compile('.*=symbols reftest/tests/layout/reftests/reftest.list.*')
        reCrashtest = re.compile('.*=symbols reftest/tests/testing/crashtest/crashtests.list.*')
        reXpcshell = re.compile('.*manifest=xpcshell/tests/all-test-dirs.list.*')
        if (reReftest.search(text)):
            return "reftest"
        elif (reCrashtest.search(text)):
            return "crashtest"
        elif (reXpcshell.search(text)):
            return "xpcshell"
        return None
        ...
        for step in buildSteps:
            if self.reParsing.search(step):
                ...
                if (doc["testtype"] <> None):
                    print "doc.testtype: " + doc["testtype"]
...
#                    break
        ...
        #TODO: figure out our structure for multiple test types.  
        # i.e.: doc['tests']['reftest'] or doc['reftest'], doc['xpcshell']
