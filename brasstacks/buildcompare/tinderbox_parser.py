...
#    db = couchquery.Database("http://happyhans:happyhanshappyhans@happyhans.couch.io/logcompare")
...
#        tbox_ids = getTinderboxIDfromDB()
    ...
    if (tbox_id in tbox_ids):
        ...
        if (result != None):
            ...
            testx = result['tests']
            count = 0
            for test in testx: count += 1
            print "counted: " + str(count) + " tests"
            save(result)


def main():
    data = getTinderboxData()
    #TODO: turn on mochitest, chrome
    testName = re.compile('((reftest)|(crashtest)|(xpcshell))')
